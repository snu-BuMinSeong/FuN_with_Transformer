...
OUT_ROOT="logs/two_stage_multiroom_n2s4/final_eval_100"
mkdir -p "${OUT_ROOT}"

echo "START week6_multiroom_seed1_final_eval $(date --iso-8601=seconds)"
...
for model in baseline ablation; do
  config="configs/finetune_fun_${model}_multiroom_n2s4_seed1.yaml"
  stage2_dir="checkpoints/two_stage_multiroom_n2s4/${model}/seed_1/stage2_argmax_ft_1000"
...
echo "ALL_DONE week6_multiroom_seed1_final_eval $(date --iso-8601=seconds)"
