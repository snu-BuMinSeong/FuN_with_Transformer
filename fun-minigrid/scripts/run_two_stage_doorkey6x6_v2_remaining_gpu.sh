...
RUN_LOG_DIR="logs/gcp_run_logs/two_stage_doorkey6x6_v2_gpu"
...
"$PY" -c 'import torch; print("cuda_available", torch.cuda.is_available()); print("device", torch.cuda.get_device_name(0))'
nvidia-smi

is_complete() {
  local log_dir="$1"
  [ -f "$log_dir/train.csv" ] || return 1
  local rows
  rows=$(($(wc -l < "$log_dir/train.csv") - 1))
  [ "$rows" -eq 5000 ]
}
...
preserve_partial() {
  ...
  if [ -d "$log_dir" ] && ! is_complete "$log_dir"; then
    mv "$log_dir" "${log_dir}_cpu_partial_${stamp}"
  fi
  if [ -d "$ckpt_dir" ] && [ ! -f "$ckpt_dir/summary.json" ]; then
    mv "$ckpt_dir" "${ckpt_dir}_cpu_partial_${stamp}"
  fi
}

preserve_partial \
  "logs/two_stage_doorkey6x6_v2/baseline/seed_44/stage1_5000" \
  "checkpoints/two_stage_doorkey6x6_v2/baseline/seed_44/stage1_5000"
preserve_partial \
  "logs/two_stage_doorkey6x6_v2/ablation/seed_1/stage1_5000" \
  "checkpoints/two_stage_doorkey6x6_v2/ablation/seed_1/stage1_5000"
...
run_batch stage1_remaining "${remaining_stage1_names[@]}"
run_batch stage2_all "${stage2_names[@]}"
