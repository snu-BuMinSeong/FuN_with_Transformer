...
OUT_ROOT="logs/multiroom_n3s5_seed1/checkpoint_eval_100"
mkdir -p "${OUT_ROOT}"

# These evaluations are small; keep them on CPU so they do not contend with GPU training jobs.
export CUDA_VISIBLE_DEVICES=""

echo "START multiroom_n3s5_checkpoint_eval $(date --iso-8601=seconds)"
...
for model in baseline ablation; do
  config="configs/train_fun_${model}_multiroom_n3s5_ms1000_seed1.yaml"
  checkpoint_dir="checkpoints/multiroom_n3s5_seed1/${model}"
...
"${PYTHON_BIN}" scripts/summarize_multiroom_n3s5_checkpoint_eval.py

echo "ALL_DONE multiroom_n3s5_checkpoint_eval $(date --iso-8601=seconds)"
