...
- [ ] baseline과 ablation이 같은 seed set을 사용했는지 확인
- [ ] baseline과 ablation이 같은 total episode 수를 사용했는지 확인
- [ ] baseline과 ablation이 같은 evaluation episode 수를 사용했는지 확인
- [ ] baseline과 ablation이 같은 `action_mode=sample` 기준으로 평가되었는지 확인
- [ ] argmax 결과가 primary metric에 섞이지 않았는지 확인
- [ ] best checkpoint evaluation 결과를 사용했는지 확인
- [ ] train/eval 로그가 오염되거나 append 문제를 일으키지 않았는지 확인

### 완료 기준

- [ ] DoorKey-5x5에서 AblationManager와 Vanilla FuN의 차이를 수치로 설명할 수 있다.
- [ ] “memory 제거 모델이 왜 더 좋게 나왔는지 / recurrent memory 이점이 왜 명확하지 않았는지”에 대한 해석 초안이 있다.
...
- [ ] 실험 환경 설명 문장
- [ ] Vanilla FuN 구조 설명 문장
- [ ] AblationManager 구조 설명 문장
- [ ] sample evaluation을 primary metric으로 둔 이유
- [ ] argmax evaluation이 실패 또는 낮게 나온 이유
- [ ] baseline 대비 ablation 결과 요약
- [ ] recurrent memory의 명확한 이점이 관찰되지 않았다는 해석
- [ ] 단순한 DoorKey-5x5 환경에서는 현재 state 기반 goal 생성만으로 충분했을 가능성
- [ ] 더 복잡한 DoorKey-6x6에서 memory 효과를 다시 확인해야 한다는 연결 문장
...
- [ ] `configs/train_fun_baseline_doorkey6x6_seed1.yaml`
- [ ] `configs/train_fun_baseline_doorkey6x6_seed11.yaml`
- [ ] `configs/train_fun_baseline_doorkey6x6_seed44.yaml`

Ablation:

- [ ] `configs/train_fun_ablation_doorkey6x6_seed1.yaml`
- [ ] `configs/train_fun_ablation_doorkey6x6_seed11.yaml`
- [ ] `configs/train_fun_ablation_doorkey6x6_seed44.yaml`
...
- [ ] seed 1 baseline/ablation config가 먼저 준비된다.
- [ ] seed 11, 44 config는 seed 1 smoke test가 통과한 뒤 확장한다.
...
- [ ] Vanilla FuN DoorKey-6x6 seed 1 smoke test
- [ ] AblationManager DoorKey-6x6 seed 1 smoke test
...
- [ ] `train.py`가 에러 없이 실행되는지 확인
- [ ] observation shape 문제가 없는지 확인
- [ ] action range 문제가 없는지 확인
- [ ] loss NaN/inf가 없는지 확인
- [ ] grad norm NaN/inf가 없는지 확인
- [ ] `train.csv` 생성 확인
- [ ] `eval.csv` 생성 확인
- [ ] `summary.json` 생성 확인
- [ ] `last.pt` 또는 checkpoint 생성 확인

### 완료 기준

- [ ] DoorKey-6x6 baseline/ablation 모두 짧은 실행이 에러 없이 끝난다.
- [ ] smoke test 로그 경로가 본실험 로그와 섞이지 않는다.
